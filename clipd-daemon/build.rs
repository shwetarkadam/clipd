//! On macOS, compile `clipd-hud` (Swift) into the same directory as other binaries
//! so `cargo build --release` works for normal users without a manual `swiftc` step.

use std::path::PathBuf;
use std::process::Command;

/// `target/{profile}/clipd-hud` — works when `CARGO_TARGET_DIR` is unset (older Cargo).
