...
impl Keyboard {
    ...
    // clipd patch (macOS 15 / Sequoia): the listen event tap runs on a
    // background thread, but TISCopyCurrentKeyboardInputSource() /
    // islGetInputSourceListWithAdditions() are main-thread-only on macOS 15+
    // and now hard-assert (dispatch_assert_queue -> SIGTRAP), crashing the whole
    // process on the first key event. We only need keycodes for hotkey
    // detection, never the translated `name` string, so skip the TSM call
    // entirely and report no name. This makes Event.name == None on macOS.
